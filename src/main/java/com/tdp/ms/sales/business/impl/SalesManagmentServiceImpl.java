...
public class SalesManagmentServiceImpl implements SalesManagmentService {
        ...
        private Boolean isValidToCallSalesEventFLow(Sale sale) {
                ...
                String flowSale = this.getStringValueByKeyFromAdditionalDataList(
                                sale.getCommercialOperation().get(0).getAdditionalData(), Constants.FLOWSALE);
                ...
                if (sale.getIdentityValidations() != null && !sale.getIdentityValidations().isEmpty()) {
                        IdentityValidationType identityValidationNoBiometric = sale.getIdentityValidations().stream()
                                        .filter(item -> item.getValidationType()
                                                        .equalsIgnoreCase(Constants.VALIDATION_TYPE_NO_BIOMETRIC))
                                        .findFirst().orElse(null);
                        if (identityValidationNoBiometric != null) {
                                String noBiometricValidationType = identityValidationNoBiometric.getAdditionalData()
                                                .stream()
                                                .filter(item -> item.getKey().equalsIgnoreCase(
                                                                Constants.VALIDATION_TYPE_NO_BIOMETRIC_ID_TYPE))
                                                .findFirst().orElse(KeyValueType.builder().value("0").build())
                                                .getValue();
                                hasNoBiometricValidation = (noBiometricValidationType.equals("1")
                                                || noBiometricValidationType.equals("2")
                                                || noBiometricValidationType.equals("3")
                                                || noBiometricValidationType.equals("4"));
                        }
                }
        ...
        public Mono<Sale> post(PostSalesRequest request) {
                ...
                Boolean isStockReserved = saleRetry[0] != null
                                ...
                                && saleRetry[0].getCommercialOperation().get(0).getDeviceOffering().get(0)
                                                .getStock() != null
                                && !StringUtils.isEmpty(saleRetry[0].getCommercialOperation().get(0).getDeviceOffering()
                                                .get(0).getStock().getReservationId());
                ...
                if (saleRetry[0] != null && saleRetry[0].getCommercialOperation().get(0).getOrder() != null
                        ...
                        if (flgCaeq[0] || flgAlta[0]) {
                                LOG.info("Sales Retry: Reserve Stock and Create Quotation");

                                return this.callToReserveStockAndCreateQuotation(
                                                PostSalesRequest.builder().sale(saleRetry[0])
                                                                .headersMap(request.getHeadersMap()).build(),
                                                saleRetry[0], flgCasi[0], flgFinanciamiento[0], sapidSimcard[0]);
                        } else {
                                if (Boolean.TRUE.equals(flgCasi[0])) {
                                        ...
                                        return this.callToCreateQuotation(
                                                        PostSalesRequest.builder().sale(saleRetry[0])
                                                                        .headersMap(request.getHeadersMap()).build(),
                                                        saleRetry[0], true, flgFinanciamiento[0]);
                                } else {
                                        // Case when is Only CAPL
                                        return salesRepository.save(saleRetry[0]).map(r -> {
                                                this.postSalesEventFlow(PostSalesRequest.builder().sale(saleRetry[0])
                                                                .headersMap(request.getHeadersMap()).build());
                                                return r;
                                        });
                                }
        ...
        private Mono<Sale> validationsAndBuildings(BusinessParametersResponse getRiskDomain,
                ...
                if (saleRequest.getCommercialOperation().get(0).getWorkOrDeliveryType() != null
                                ...
                                && saleRequest.getCommercialOperation().get(0).getWorkOrDeliveryType()
                                                .getMediumDelivery().equalsIgnoreCase("DELIVERY")
                                && saleRequest.getPaymenType().getPaymentType().equalsIgnoreCase("EX") && this
                                                .getStringValueByKeyFromAdditionalDataList(
                                                                saleRequest.getAdditionalData(), "paymentTypeLabel")
                                                .equals("PAGO EFECTIVO")) {
        ...
        private void buildCreateQuotationRequest(CreateQuotationRequest createQuotationRequest,
                ...
                Double taxExcludedAmountDouble = sale.getCommercialOperation().get(0).getDeviceOffering().get(0)
                                ...
                                .getFinancingInstalments().get(0).getInstalments().getTotalAmount().getValue()
                                .doubleValue() * 0.18;
                MoneyAmount taxExcludedAmount = MoneyAmount.builder().amount(taxExcludedAmountDouble.toString())
                                .units("PEN").build();
                ...
                com.tdp.ms.sales.model.dto.quotation.Item itemEquipment = com.tdp.ms.sales.model.dto.quotation.Item
                                ...
                                .totalCost(totalCost).orderActionId(sale.getCommercialOperation().get(0).getOrder()
        ...
        public CreateProductOrderGeneralRequest caeqCommercialOperation(Sale saleRequest,
                ...
                NewProductCaeq newProductCaeq1 = NewProductCaeq.builder()
                                .productId(saleRequest.getCommercialOperation().get(0).getProduct().getId()) // Consultar
                                                                                                             // porque
                                                                                                             // hay 2
                                                                                                             // product
                                                                                                             // ids
        ...
        private Mono<Sale> wirelineMigrations(List<BusinessParameterFinanciamientoFijaExt> parametersFinanciamientoFija,
                ...
                migracionFijaRequest.setCip(request.getSale().getPaymenType() != null
                                && !StringUtils.isEmpty(request.getSale().getPaymenType().getCid())
                                                ? request.getSale().getPaymenType().getCid()
                                                : null);
                migracionFijaRequest.setUpfrontIndicator(request.getSale().getCommercialOperation().get(0)
                                .getProductOfferings().get(0).getUpFront().getIndicator());
